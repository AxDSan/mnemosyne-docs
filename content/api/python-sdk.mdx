...
| `author_id` | `str \| None` | `None` | Author identifier for multi-agent identity  |
| `author_type` | `str \| None` | `None` | Author category: `"human"`, `"agent"`, or `"system"`  |
| `channel_id` | `str \| None` | `None` | Channel for cross-session shared memory. Defaults to `session_id`  |
...
| `author_id` | `str \| None` | `None` | Filter by specific author  |
| `author_type` | `str \| None` | `None` | Filter by author category: `"human"`, `"agent"`, `"system"`  |
| `channel_id` | `str \| None` | `None` | Filter by channel for cross-session recall  |
...
Returns a dict with per-session consolidation and degradation summaries.
...
The `Mnemosyne` instance exposes several lazy-initialized subsystems:
...
| `plugins` | `PluginManager` | Discovers and manages plugins from `~/.hermes/mnemosyne/plugins/`. Built-in: `LoggingPlugin`, `MetricsPlugin`, `FilterPlugin`, `CompressionPlugin` |
...
<CodeBlock code={`from mnemosyne.core.triples import TripleStore
...
## Hermes Plugin Tools (31 tools)

<Callout type="info">
The Hermes plugin registers 31 tools. Shared surface tools (`shared_remember`, `shared_recall`, `shared_forget`, `shared_stats`) and `validate`/`get` are MCP-only and not registered by the plugin.
</Callout>
