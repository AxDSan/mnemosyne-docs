...
**Last updated:** July 2026 &middot; Mnemosyne v3.14.0
...
mnemosyne sync --db-path <surface.db> --remote https://my-vps.example.com:8765

# Pull only (fetch remote changes without pushing local)
mnemosyne sync --db-path <surface.db> --remote https://my-vps.example.com:8765 --mode pull

# Push only (send local changes without fetching)
mnemosyne sync --db-path <surface.db> --remote https://my-vps.example.com:8765 --mode push`} language="bash" />
...
MNEMOSYNE_SYNC_KEY=$(cat mnemosyne-sync.key) \
  mnemosyne sync --db-path <surface.db> --remote https://my-vps.example.com:8765 --encrypt`} language="bash" />
...
# Client side
export MNEMOSYNE_SYNC_TOKEN="***"
mnemosyne sync --db-path <surface.db> --remote https://my-vps:8765`} language="bash" />
...
# Client side
MNEMOSYNE_SYNC_TOKEN="<token>" mnemosyne sync --db-path <surface.db> --remote https://my-vps:8765`} language="bash" />
...
mnemosyne sync --db-path <surface.db> --remote https://my-vps:8765 --encrypt

# Or use a passphrase (key derived automatically)
export MNEMOSYNE_SYNC_KEY="your strong passphrase"
mnemosyne sync --db-path <surface.db> --remote https://my-vps:8765 --encrypt`} language="bash" />
...
services:
  mnemosyne-sync:
    ...
    environment:
      ...
      - MNEMOSYNE_SYNC_KEY=/run/secrets/sync.key
    command: >
...
mnemosyne sync --db-path <surface.db> --remote http://localhost:8765`} language="bash" />
